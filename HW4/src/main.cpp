    ...
    while(input_file.getline(temp_char, 200)) {
        ...
        switch(temp_char[0]) {
            case 'E':
            {
                ...
                vec4 tempVec(temp[0], temp[1], temp[2], 1.0f);
                rt->set_eye(tempVec);
                break;
            }
            case 'V':
            {
                vector<float> temp = get_float(temp_string);
                vec4 tempVec(temp[0], temp[1], temp[2], 0.0f);
            ...
            {
                vector<float> temp = get_float(temp_string);
                rt->create_triangle(temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], temp[7], temp[8], temp[9], temp[10], temp[11]);
                break;
            }
            case 'L':
            {
